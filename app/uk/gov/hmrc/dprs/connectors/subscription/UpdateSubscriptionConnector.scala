...
@Singleton
class UpdateSubscriptionConnector @Inject() (appConfig: AppConfig, wsClient: WSClient, clock: Clock) extends BaseBackendConnector(wsClient, clock) {
  ...
  override def baseUrl(): URL = url"${appConfig.updateSubscriptionBaseUrl}"
